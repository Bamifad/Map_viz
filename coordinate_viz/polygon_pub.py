...
def StarPolygon(header):
    p = PolygonStamped()
    p.header = header
    p.polygon.points = [Point(x=.0000, y=1.0000 + 3.0),
                        Point(x=.2245, y=.3090 + 3.0),
                        Point(x=.9511, y=.3090 + 3.0),
                        Point(x=.3633, y=-.1180 + 3.0),
                        Point(x=.5878, y=-.8090 + 3.0),
                        Point(x=.0000, y=-.3820 + 3.0),
                        Point(x=-.5878, y=-.8090 + 3.0),
                        Point(x=-.3633, y=-.1180 + 3.0),
                        Point(x=-.9511, y=.3090 + 3.0),
                        Point(x=-.2245, y=.3090 + 3.0)]
...
if __name__ == "__main__":
    ...
    pub = rospy.Publisher("~output", PolygonArray, queue_size=100)
    r = rospy.Rate(10)
